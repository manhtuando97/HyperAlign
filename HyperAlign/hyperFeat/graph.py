...
def load_hyperedge(file):
  ...
  for line in f:
    ...
    for i in range(len(lines)):
      ...
      if G[x] is None:
        G[x] = []
      if x not in incidence_size.keys():
        incidence_size[x]  = dict()
        for size in range(2,11):
          incidence_size[x][size] = 0
      
      if len(lines) < 11:
        incidence_size[x][len(lines)] += 1
...
# changed to items from iteritems